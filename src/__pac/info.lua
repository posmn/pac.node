...
local _info = {
    pacprotocold = _status,
    started = _started,
    level = 'ok',
    synced = false,
    status = 'Paccoin node down',
...
local function _exec_paccoin_cli(...)
    ...
    local _proc = proc.spawn('bin/pacprotocol-cli', _arg, {stdio = {stdout = 'pipe', stderr = 'pipe'}, wait = true})
...
local function _get_paccoin_cli_result(exitcode, stdout, stderr)
    if exitcode ~= 0 then
...
if _info.pacprotocold == 'running' then
    if am.app.get_configuration("NODE_PRIVKEY") then
        local _exitcode, _stdout, _stderr = _exec_paccoin_cli("-datadir=data", "masternode", "status")
        local _success, _output = _get_paccoin_cli_result(_exitcode, _stdout, _stderr)
    ...
    local _exitcode, _stdout, _stderr = _exec_paccoin_cli('-datadir=data', 'getblockchaininfo')
    local _success, _output = _get_paccoin_cli_result(_exitcode, _stdout, _stderr)
    ...
    local _exitcode, _stdout, _stderr = _exec_paccoin_cli('-datadir=data', 'mnsync', 'status')
    local _success, _output = _get_paccoin_cli_result(_exitcode, _stdout, _stderr)
